...
local function lookup(ip)
    ...
    local command = "grep \"\\b" .. ip .. "\\b\" " .. lease_file .. " | awk '{print $4}'"
...
function read()
    ...
    if not pjson or not pjson.data then
        collectd.log_error("nlbw2collectd: Failed to parse JSON or got no data. Raw output: " .. tostring(json))
        return 0
    end
